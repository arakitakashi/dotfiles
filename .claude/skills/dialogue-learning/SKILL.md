...
- 解答例は折りたたみ（`<details>`）で章末に置くか、付録にまとめる

#### 4-6-1. 折りたたみ（`<details>`）の書き方【Obsidian で必ず守る】

成果物は Obsidian で閲覧される。`<details>` の書式を誤ると **トグルが壊れる / 生タグが文字として漏れる**。以下を厳守する。

- **`<details>` を引用ブロック（`>`）の中に入れない。** `>` の中に `<details>` があると、空行で HTML ブロックが分断され、開きタグと閉じタグが別 DOM 断片に入って入れ子が壊れる。演習の問題文を `>` の引用にした場合は、**`<details>` は引用の外**に出す。
- **`<details>` の前に空行**を1つ置く。
- **`<details>` / `<summary>…</summary>` / `</details>` は各行に分ける。** `<details><summary>…</summary>` のように1行に詰めない。
- **`<summary>` の直後と `</details>` の直前に空行**を入れる（中の Markdown を描画させるため）。
- 解答が複数項目（A1/A2…）なら **項目間にも空行**を入れて別段落にする。
...
- [ ] `<details>` を引用ブロック（`>`）の **外** に置いた（`>` の中に入れていない）
- [ ] `<details>` の前に空行があり、`<details>`/`<summary>`/`</details>` を各行に分け、`<summary>` 直後と `</details>` 直前に空行を入れた
- [ ] 解答が複数項目のとき、項目間に空行を入れて別段落にした
