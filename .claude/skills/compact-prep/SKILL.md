...
argument-hint: "[復旧メモ]"
allowed-tools: Read, Write, Bash(~/.claude/scripts/get-session-id.sh:*), Bash(echo:*), Bash(mkdir:*), Bash(date:*), Bash(pwd)
---
...
3. TaskList、active plan file、並走 worker の状態、編集中ファイルを確認する。
   - active plan file がある場合は該当ファイルを読む（`~/.claude/plans/` 配下など）。
