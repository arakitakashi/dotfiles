...
name: compact-prep
description: |
  Codex の /compact 実行前に、現セッションの作業状態を一時 state file へ保存する。
...
argument-hint: "[復旧メモ]"
allowed-tools: Read, Write, Bash(~/.Codex/scripts/get-session-id.sh:*), Bash(echo:*), Bash(mkdir:*), Bash(date:*), Bash(pwd)
---

# compact-prep

Codex の `/compact` 前に、圧縮サマリーへ残りにくい作業状態を
`${TMPDIR}/Codex-compact-state/${SESSION_ID}.md` へ保存する。
...
1. session_id を取得する。
   - `~/.Codex/scripts/get-session-id.sh` を実行する。
   - 取得できない場合は state file を作らず、session_id が取得できないため準備未完了と報告する。
2. 保存先を `${TMPDIR:-/tmp}/Codex-compact-state/${SESSION_ID}.md` に決める。
   - `echo "${TMPDIR:-/tmp}"` で TMPDIR の実パスを確認し、Write には絶対パスを使う。
3. TaskList、active plan file、並走 worker の状態、編集中ファイルを確認する。
   - active plan file がある場合は該当ファイルを読む（`~/.Codex/plans/` 配下など）。
   - 並走 worker（tmux 等）を使っていない場合は「未使用」と記録する。
...
- 制約、ブロッカー、未完了の検証
- worker 体制。tmux 等の並走環境使用時は pane、role、担当を記録する
- 編集中のファイルと、未保存または未検証の注意点
- 圧縮後の自分への復旧メモ（引数で復旧メモが渡された場合は必ず含める）
